...
const customKey = ['custom_get_actions'];
...
exports.main = async (event, _context) => {
  ...
  if (!_.isEmpty(event)) {
    ...
    let { action, params, cookie, debug } = data;
    const { proxy } = data;
    if (!action) {
      return apiResponse(isSdkRequest, 500, 'action not found', {
        event,
        data,
        isSdkRequest,
      });
  ...
  return apiResponse(isSdkRequest, 500, 'event not found', {
    event,
    isSdkRequest,
  });
};
